// flush-scenes.jsx — Flush MVP voice-capture prototype scenes.
// Three calming variations, each a full landing → recording → completion flow.
// Mounted inside window.IOSDevice (from ios-frame.jsx).
// Exports to window: FlushWeather, FlushOrb, FlushWater
...
(function () {
  ...
  // ---------- shared state hook ----------
  function useFlow() {
    const [phase, setPhase] = R.useState('idle'); // idle | recording | completing | summary | reflect
  ...
  function DoneButton({ onClick, tone }) {
    // tone: 'light' (dark text on translucent) | 'dark' (light text)
    const [hov, setHov] = R.useState(false);
  ...
  // gentle "listening" indicator — three breathing bars, not a meter
  ...
  // ---------- Shazam-style circular tap target ----------
  // Big central button with continuous concentric pulse rings.
  function ShazamButton({ onClick, size = 132, ringColor, core, coreColor, coreBorder, glyph, rings = 3, spin = false, glass = false }) {
    const [hov, setHov] = R.useState(false);
    return h('button', {
      onClick, onMouseEnter: () => setHov(true), onMouseLeave: () => setHov(false),
      ...
      h('div', {
        ...
        style: {
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          overflow: 'hidden',
          backdropFilter: glass ? 'blur(14px) saturate(1.6)' : 'none', WebkitBackdropFilter: glass ? 'blur(14px) saturate(1.6)' : 'none',
          boxShadow: glass
  ...
  // sound-wave glyph — vertical equalizer bars (symmetric, rounded caps)
  function WaveGlyph({ color = '#fff', size = 52 }) {
    // heights as fraction of viewBox, symmetric around center
  ...
  // Headline that gently cycles through prompts with a fade + soft rise.
  function CyclingHeadline({ prompts, color }) {
    const [i, setI] = R.useState(0);
    const [vis, setVis] = R.useState(true);
    R.useEffect(() => {
      let outT;
      const inT = setTimeout(() => {
        setVis(false);                       // fade current out
        outT = setTimeout(() => {            // swap + fade next in
  ...
  // ---------- Canvas particle weather system ----------
  // phase: 'idle' | 'recording' | 'completing' | 'summary'
  // Soft volumetric cloud puffs drift; rain streaks + splashes during recording;
  // clouds part and a warm sun glows on completion. Runs its own rAF loop so
  // motion survives React re-renders (phase read via ref).
  function WeatherCanvas({ phase }) {
    const ref = R.useRef(null);
    const phaseRef = R.useRef(phase);
    ...
    R.useEffect(() => {
      ...
      // ---- build clouds: each is a cluster of soft puffs ----
      const makeCloud = (cx, cy, scale, speed, depth) => {
        ...
        for (let i = 0; i < n; i++) {
          ...
          puffs.push({
            ...
            ph: rnd(0, Math.PI * 2),          // churn phase
            sp: rnd(0.6, 1.3),                // churn speed
            amp: rnd(4, 10) * scale,          // churn drift amplitude
          });
        }
      ...
      // stagger arrival + assign which edge each cloud drifts in from
      clouds.forEach((c, i) => {
        c.delay = i * 3.4;                  // long, gradual filling (~40s to full)
        c.side = c.cx < 0.5 ? -1 : 1;       // enter from nearest edge
      });
      ...
      // ambient floating motes (dust-of-light) — only while listening
      const motes = [];
      ...
      let rainI = 0;      // eased rain intensity 0..1
      let clear = 0;      // eased clearing 0..1
      let formed = 0;     // eased cloud presence (0 at idle → 1 once talking begins)
      ...
      const spawnRain = (dt) => {
        const rate = rainI * 3.2; // particles per frame-ish
        let count = rate;
        while (count > 0) {
      ...
      const draw = (now) => {
        ...
        // clouds — during 'gather' (recording) they grow fuller, converge and rise
        const drawClouds = () => {
        for (const c of clouds) {
          // gradual staggered arrival: each cloud slides in from its edge over ~9s,
          // and the last of 10 clouds finishes around the 40s mark
          ...
          const eased = 1 - Math.pow(1 - c.prog, 3);   // easeOutCubic slide
          const opac = Math.min(c.prog * 7, 1);         // opaque early so it's visible AT the edge
          c.drift += c.speed * dt * (20 + gather * 6);        // keep drifting, faster while gathering
          const sway = Math.sin(c.drift * 0.5) * (12 + gather * 4) * c.scale;
          const conv = gather * (0.5 - c.cx) * W * 0.28;      // drift toward center
          const part = clear * (c.cx < 0.5 ? -1 : 1) * W * 0.5 * c.depth;
          const enterX = (1 - eased) * c.side * W * 1.15;     // start fully offscreen, slide to place
          const baseX = c.cx * W + sway + conv + part + enterX;
          const baseY = c.cy * H - gather * H * 0.02 + Math.sin(c.drift * 0.32) * 6 * c.scale;
          const grow = 1 + gather * 0.42;                      // puffs swell
          const churn = 0.4 + gather * 0.6;                    // billow more while listening
        ...
        // ambient light rays + floating motes while listening (fade with gather)
        const amb = gather * (1 - clear);
        if (amb > 0.02) {
          // soft god-rays fanning down from top center
          ctx.save();
        ...
        // clouds render ABOVE the motes/rays
        drawClouds();
  ...
  // ---------- Summary of key topics (shown after the completion animation) ----------
  function SummaryView({ onReflect, onDone }) {
    // weight drives bubble size; each theme gets a soft accent tint
    const initial = [
    ...
    const [items, setItems] = R.useState(initial);
    const [drag, setDrag] = R.useState(null);
    const refs = R.useRef([]);
    const holdT = R.useRef(null);
    const beginHold = (i, e) => {
  ...
  // ---------- Reflection prompt (optional, after the summary) ----------
  // A calm SVG mood face (drawn, not emoji — stays on-brand)
  ...
  function MoodPicker({ onPick }) {
    ...
    return h('div', { style: { position: 'absolute', inset: 0, zIndex: 7, background: 'radial-gradient(120% 80% at 50% 20%,#FFFDF6 0%,#FBF4E6 60%,#F5ECDA 100%)', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '104px 26px 60px', boxSizing: 'border-box', animation: `flush-rise .6s ${EASE} both` } }, [
      ...
      h('div', { key: 'row', style: { display: 'flex', justifyContent: 'center', gap: 16 } }, moods.map((m, i) => h('button', {
        ...
        'style-hover': { transform: 'translateY(-4px)', boxShadow: '0 10px 24px rgba(76,29,68,.12)' },
        'style-active': { transform: 'scale(.96)' },
      }, [
  ...
  function ReflectView({ onDone }) {
    const [mood, setMood] = R.useState(null);
  ...
  function WeatherScene(props) {
    ...
    const cloud = null, rain = null, sun = null;
  ...
  function FlushWeather() {
    return h(window.IOSDevice, {}, h(WeatherScene));
  }

  // ---------- Digest activity grid (time-of-day × day-of-week heatmap) ----------
  function DigestGrid() {
    // theme palette
    const themes = {
    ...
    // filled cells use the theme color; a light-tint fill for the softer occurrences
    const intensity = {
      dawn:  { sleep: 1 },
  ...
  // ---------- Reflections tree (tap a leaf to read that day's note) ----------
      ...
      h('svg', { key: 'svg', viewBox: `0 0 ${W} ${H}`, width: '100%', style: { display: 'block', overflow: 'visible' } },
        branches.flatMap((b, bi) => {
          ...
          b.leaves.forEach((lf, li) => {
            const lx = jx + lf.dx * W;
            const ly = topY + (1 - lf.t) * 34;   // taller leaves sit higher
  ...
  // ---------- Digest screen (review past Flush sessions) ----------
  function DigestScreen() {
    ...
    return h('div', { style: { position: 'absolute', inset: 0, zIndex: 3, background: 'linear-gradient(180deg,#FCFCFB 0%,#F5EFE4 100%)', overflow: 'auto', padding: '76px 24px 128px', boxSizing: 'border-box', animation: `flush-rise .5s ${EASE} both` } }, [
      ...
      h('div', { key: 'cta', style: { marginTop: 22, background: '#4C1D44', borderRadius: 24, padding: '26px 24px 28px', position: 'relative', overflow: 'hidden' } }, [
        ...
        h('button', { key: 'b', style: { position: 'relative', appearance: 'none', cursor: 'pointer', border: 'none', background: '#FFFFFF', color: '#010204', borderRadius: 9999, padding: '15px 26px', display: 'inline-flex', alignItems: 'center', gap: 12, fontFamily: '"Season Sans",system-ui', fontSize: 15.5, fontWeight: 600, transition: `transform .25s ${EASE}` }, 'style-hover': { transform: 'translateY(-2px)' }, 'style-active': { transform: 'scale(.97)' } }, [
  ...
  // ---------- Bottom tab switcher (subtle segmented pill) ----------
  ...
  // 2a — Weather + Shazam tap circle, wrapped in a Flush / Digest tabbed app
  function FlushWeatherTap() {
    return h(window.IOSDevice, {}, h(FlushTabApp));
  }

  function FlushTabApp() {
    const [tab, setTab] = R.useState('flush');
    const flow = useFlow();
  ...
  // ============================================================
  // 1b — BREATHING ORB (night → dawn)
  // ============================================================
  function OrbScene(props) {
    const { phase, start, done, reset } = useFlow();
    const recording = phase === 'recording';
    const clearing = phase === 'completing';
    ...
    return h('div', { style: { position: 'absolute', inset: 0, background: bg, transition: `background 1.6s ${EASE}`, overflow: 'hidden' } }, [
      stars,
      // orb cluster centered upper-middle
  ...
  function FlushOrb() {
    return h(window.IOSDevice, { dark: true }, h(OrbScene));
  }

  // 2b — Orb + "tap the light" glowing shazam circle
  function FlushOrbTap() {
    return h(window.IOSDevice, { dark: true }, h(OrbScene, {
      hideIdleOrb: true,
      renderIdle: ({ start }) => [
  ...
  // ============================================================
  // 1c — STILL WATER (editorial ripples)
  // ============================================================
  ...
  function FlushWater() {
    return h(window.IOSDevice, {}, h(WaterScene));
  }

  // 2c — Water + hollow "press" ring that drops ripples
  function FlushWaterTap() {
    return h(window.IOSDevice, {}, h(WaterScene, {
      renderIdle: ({ start }) => [
  ...
  Object.assign(window, { FlushWeather, FlushOrb, FlushWater, FlushWeatherTap, FlushOrbTap, FlushWaterTap });
})();
